...
2. **Every generated reply.** `produceLiveAppReply` still classifies with Jev, then the bake-off tier writes a draft. `jevQualityRewrite` asks Jev on `/api/alpha/decisions` (`typesafe/jev-1.13`) for a score 1–5, a comment, and keep-or-rewrite. When Jev chooses rewrite, the same bake-off tier rewrites under that comment, and that text is the customer-facing reply if it still obeys the destination lock, the Item34 ban, and the garden wording. The stored turn keeps `quality.judged`, `quality.score`, `quality.comment`, and `quality.rewritten`. A reply Jev does not score is not stored. The Dialog PDF line is `quality: <score> — <comment>` and adds `(rewritten)` when the rewrite was accepted. `quality: not judged` is refused.
