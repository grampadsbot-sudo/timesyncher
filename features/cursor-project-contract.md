# Cursor Project — Style one / Style two contract

GBrain: `bot-admin/messages/time-syncher/cursor-project-style-two-contract-20260916`

Repo files (always on for agents):

- `AGENTS.md`
- `.cursor/rules/style-two-keepsake-contract.mdc`
...
Saved Story is a **flag on the Thing**. Product Style one / Style two `Ae()` print code always embeds that Thing’s bound media (`places[].bound_media` / `/ts-thing-media`) as **actual JPEG bytes** (`printDataUrl`). Cursor memory is not a substitute. Placeholders while those URLs 200 = FAIL. Implementation: `src/vacation/trek-style2-bundle.mjs`.
