...
- **TREK id-set movement.** Run `node scripts/control-vacation.mjs apply --trek-db /tmp/vacation-trek-verify.db --fixture features/fixtures/telegram-text-single-edit.json --json`. Mode is `apply_trek_sqlite`. Bellagio `placeDay` becomes `2`. `trek_state.row_ids_before` equals `row_ids_after` and row count stays `1`.
...
- Live Telegram intake (`api/vacation-telegram-turn.mjs`, `scripts/telegram-vacation-intake-bot.mjs`, `scripts/product-gbrain-dispatch.mjs`) calls `gateTelegramIntakeEdit`. Integrity fail-closed stops writers. Item matching and hosted TREK mutation still happen in `product-gbrain-dispatch` / `trek-itinerary-edit` when intake has no Thing list.
