...
- Live Telegram intake (`api/vacation-telegram-turn.mjs`, `scripts/telegram-vacation-intake-bot.mjs`, `scripts/product-gbrain-dispatch.mjs`) calls `gateTelegramIntakeEdit`. Empty Thing list, unmatched edits, skip, and integrity stops fail closed with `editApplied: false` and never call `applyTrekItineraryEdit`. `pipelineWriteDecision.allowTrekWrite` is the only path to a TREK writer.
