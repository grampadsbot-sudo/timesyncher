...
- **Proof.** Refresh the committed inspectable receipt with `node scripts/control-vacation.mjs commit-proof`. Doctor re-execs the fixture and requires `proof_digest` on `receipt.json` and `dry-run.json` to match the live run (plus before/after hashes and event payloads). A rewritten `generated_at` / event `ts` is not freshness. `apply_gate.prove_state_movement` is `hold` and `apply_on_this_receipt` is `false` on this dry-run tree. Read `features/proof/vac-verify-telegram-text-single-edit/{receipt.json,events.jsonl,dry-run.json}` in the repo. Do not treat a gitignored `artifacts/` run as in-tree proof.
...
- Live Telegram intake (`api/vacation-telegram-turn.mjs`, `scripts/telegram-vacation-intake-bot.mjs`, `scripts/product-gbrain-dispatch.mjs`) calls `gateTelegramIntakeEdit`. Empty Thing list, unmatched edits, skip, and integrity stops fail closed with `editApplied: false` and never call `applyTrekItineraryEdit`. `pipelineWriteDecision.allowTrekWrite` is the only path to a TREK writer. After that gate, `applyTrekItineraryEdit` and the FORCE/agent path apply pipeline `planned_writes` only (`applyValidatedOnly`); they must not re-parse `requestText`. Once the turn gate has move/remove `planned_writes`, it does not queue a write worker and must not reply `Moved`/`Removed` — that copy claims an update without TREK movement. The turn fail-closes with `apply_not_on_turn`. Collaborator/owner `add_thing` writes keep the same no-apply turn copy and queue `applyExistingTripEdit` so new restaurants/stores/flights can land on TREK tabs. Live TREK apply is a separate entry: worker `applyExistingTripEdit` for queued first-pass/non-edit or add_thing jobs, or `control-vacation apply --trek-db` / `--local-snapshot`. Do not add a second writer on the turn. The Thing list is live-locked `trip_things` for that `trip_id`, not client `payload.things`. The bot assigns `payload.liveSession` from `resolve_live_session` so unauthorized blocking is not inert.
