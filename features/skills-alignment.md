# GBrain skills this map implements

Completer/skillify jobs 18–22 in flight. Hold certify. Do not claim Completer 10/10 without `gbrain skillify check --require-completer` + dated receipt.

| Skill | Role |
| --- | --- |
| `skills/gbrain-canonical-actions` | GBrain skills first; ignore conflicting bot-local skills (CoS job 9) |
| `skills/style-2-journey-book-export` | Sole SoT `style-2-journey-book-standard`; Layout 2 / Style two only |
| `skills/keepsake-qa` | Fail-closed Style two Config honor + live Feature Map + **budget** + **per-category fill 15/10/15** |
| `skills/thing-logo-icon-rules` | Logos at create; airplane flights-only; missing → category/default |
| `skills/vacation-media-bind` | Neon bind API; no Blob; no tokens in chat/PR |
| `skills/feature-map-full-ui-inventory` | Every live UI control in `features/`; stamp Config dump |
| `skills/tg-intake-gbrain-track-and-build` | Persist TG Q&A in GBrain; emit 10–15 min build cue |
| `.cursor/skills/verify-timesyncher-vacation` | Post-purchase harness: email launch, then EULA inside the app URL, then chat. Fails if evidence skips email or EULA is only on order-success |

**Sole SoT:** `bot-admin/messages/time-syncher/style-2-journey-book-standard`  
Config dump: `bot-admin/messages/time-syncher/keepsakes-config-defaults-20260910`  
Autonomy: `bot-admin/messages/time-syncher/autonomous-app-customer-flow-20260910` (Telegram flow → GBrain + code + website)  
Without bots: `bot-admin/messages/time-syncher/autonomous-app-without-bots-20260910` (do not wait on OpenClaw)  
Keepsake QA: `bot-admin/messages/time-syncher/keepsake-qa-site-and-config-20260910` — travel Style two `zu()` **FAIL**; hold certify.
