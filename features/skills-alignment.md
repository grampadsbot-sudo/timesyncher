# GBrain skills this map implements

Completer/skillify jobs 18–22 in flight. Hold certify. Do not claim Completer 10/10 without `gbrain skillify check --require-completer` + dated receipt.
...
| `skills/keepsake-qa` | Fail-closed Style two Config honor + live Feature Map + **budget** + **per-category fill 15/10/15** |
...
| `.cursor/skills/verify-timesyncher-vacation` | Post-purchase harness: email launch, then EULA inside the app URL, then chat. Fails if evidence skips email or EULA is only on order-success. Live composer harness: Jev then tier on stored turns (`verify-live-app-jev-tier.mjs`) |
