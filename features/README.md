...
- SHA evidence for the reviewer commands is both GitHub Actions jobs on this SHA: `vacation-verify` and `vacation-verify-doctor` with `conclusion=success`, plus each job's artifact `sha256:` digest. Harness green alone is not attestation. Doctor parses `run:` steps (not whole-file includes). A committed `features/proof/vac-verify-ci/receipt.json` cannot skip the API; if present it must match the live `run_id` and artifact digest. `in_progress` is not success. Fail-closed if either job or digest is missing. The harness job uploads fixture `events.jsonl` / `proof_digest` receipts; doctor runs after that job and binds both digests. Vercel `timesyncher` / timesyncher.com is a named deploy; doctor and dry-run have no remote target — do not invent one. A Vercel commit status is not vacation-verify.
