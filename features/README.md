...
- SHA evidence for the reviewer commands is the harness job `vacation-verify` plus `vacation-verify-doctor` and `vacation-verify-gate`, each `conclusion=success`. Doctor proof is the `vacation-verify-gate-${sha}` artifact (`doctor.json`), not the `vacation-verify-doctor-${sha}` marker. Harness green or marker-only is not attestation. Mid-job `in_progress` or a null gate digest is not attestation. Doctor parses `run:` steps (not whole-file includes). A committed receipt cannot skip the API; if present it must include `doctor_artifact_digest` equal to the live **gate** digest. `vacation-verify-attest` runs doctor after the gate artifact exists. Vercel `timesyncher` / timesyncher.com is a named deploy; doctor and dry-run have no remote target — do not invent one. A Vercel commit status is not vacation-verify.
