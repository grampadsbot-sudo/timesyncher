...
- SHA evidence for the reviewer commands is the harness job `vacation-verify` plus `vacation-verify-doctor`, `vacation-verify-gate`, and `vacation-verify-attest`, each `conclusion=success`. Doctor proof is the `vacation-verify-gate-${sha}` artifact (`doctor.json` with `ok:true`), not the `vacation-verify-doctor-${sha}` marker. Gate fails if doctor exits non-zero or the uploaded JSON is not `ok:true`. Harness green or marker-only is not attestation. Mid-job `in_progress` or a null gate digest is not attestation. Deleting or skipping `vacation-verify-attest` fails attestation; its `vacation-verify-attest-${sha}` digest is bound when a receipt is present. Doctor parses `run:` steps (not whole-file includes). A committed receipt cannot skip the API; if present it must include `doctor_artifact_digest` equal to the live **gate** digest and `attest_artifact_digest` equal to the live attest digest. Vercel `timesyncher` / timesyncher.com is a named deploy; doctor and dry-run have no remote target — do not invent one. A Vercel commit status is not vacation-verify.
