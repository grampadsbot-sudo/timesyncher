...
- SHA evidence for the reviewer commands is both GitHub Actions jobs on this SHA: `vacation-verify` and `vacation-verify-doctor` with `conclusion=success`, plus each job's artifact `sha256:` digest. Harness green alone is not attestation. `GITHUB_JOB=vacation-verify-doctor` mid-job (`in_progress` or null doctor digest) is not attestation. Doctor parses `run:` steps (not whole-file includes). A committed `features/proof/vac-verify-ci/receipt.json` cannot skip the API; if present it must include `doctor_artifact_digest` and match both live artifact digests. Fail-closed if either job or digest is missing. The harness job uploads fixture receipts; the doctor job uploads its artifact; `vacation-verify-gate` runs doctor after both jobs and both digests exist. Vercel `timesyncher` / timesyncher.com is a named deploy; doctor and dry-run have no remote target — do not invent one. A Vercel commit status is not vacation-verify.
