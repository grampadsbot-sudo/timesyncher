...
- SHA evidence for the reviewer commands is the harness job `vacation-verify` plus `vacation-verify-doctor`, `vacation-verify-gate`, `vacation-verify-attest`, and `vacation-verify-bind`, each `conclusion=success`. Default `doctor` requires `ci_attestation`; `--produce-gate` / `--produce-attest` cannot set `ok:true` when attestation fails. CI runs that default doctor in `vacation-verify-bind`. Doctor proof is a full `doctor.json` (required checks + job ids + harness/gate/attest/bind digests), not shallow `{ok:true}` and not the doctor-job marker. `doctor_job_id` is `vacation-verify-doctor`. Mid-bind uploaded JSON may omit the bind digest while `bind_conclusion=in_progress`; a completed report without `bind_job_id` / bind digest fail-closes. Gate fails if doctor exits non-zero or the uploaded JSON is not `ok:true`. Harness green or marker-only is not attestation. Mid-job `in_progress` or a null gate/attest digest is not attestation. Deleting or skipping `vacation-verify-attest` fails attestation. Doctor parses `run:` steps (not whole-file includes). A committed CI receipt at [proof/vac-verify-ci/receipt.json](./proof/vac-verify-ci/receipt.json) is required. Missing or mismatched run_id / job conclusions / harness-gate-attest-bind digests, or a receipt that omits `vacation-verify-bind`, fail-closed. Two-proof attest: the receipt SHA must be HEAD or the direct parent of HEAD (one-commit lag max). Deeper ancestors fail-closed. Doctor JSON must name both proofs (`receipt_lag` plus `committed_receipt` digests). Same-SHA receipts must match live HEAD bind digest; parent-lag receipts must not alias that digest. Completed parent-lag fails if the live HEAD bind digest or run is missing; mid-bind `in_progress` may omit the live bind digest. `doctor_artifact_*` is the gate zip (`doctor_artifact_kind=gate`). The `vacation-verify-doctor` zip is marker-only (`doctor_job_artifact_kind=marker`). The current tree receipt binds `9a98cab` / run `33823585754`. Do not rewrite the receipt onto every new HEAD. The receipt cannot skip the live HEAD attestation and cannot stand in for a live HEAD bind. Vercel `timesyncher` / timesyncher.com is a named deploy; doctor and `control-vacation` dry-run have no remote target. Hosted PR-preview probes use `node scripts/feature-map-preview.mjs` / `features/preview-map.json` and are not doctor attestation. A Vercel commit status is not vacation-verify.
...
## Hosted preview probe (not doctor)

`node scripts/feature-map-preview.mjs` hits Feature Map user-facing pages and fail-closed APIs on a named PR preview using `VERCEL_PROTECTION_BYPASS` (header only; never logged). `extract` turns a sanitized HAR into deterministic `features/preview-map.json`. Playwright is optional for rendered visits. Do not POST payment, coupon, or Telegram turn from this lever.
