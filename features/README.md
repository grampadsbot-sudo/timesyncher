...
- SHA evidence for the reviewer commands is the harness job `vacation-verify` plus `vacation-verify-doctor`, `vacation-verify-gate`, and `vacation-verify-attest`, each `conclusion=success`. Default `doctor` requires `ci_attestation`; `--produce-gate` / `--produce-attest` cannot set `ok:true` when attestation fails. CI runs that default doctor in `vacation-verify-bind`. Doctor proof is a full `doctor.json` (required checks + job ids + digests), not shallow `{ok:true}` and not the doctor-job marker. `doctor_job_id` is `vacation-verify-doctor`. Gate fails if doctor exits non-zero or the uploaded JSON is not `ok:true`. Harness green or marker-only is not attestation. Mid-job `in_progress` or a null gate/attest digest is not attestation. Deleting or skipping `vacation-verify-attest` fails attestation. Doctor parses `run:` steps (not whole-file includes). A committed CI receipt at [proof/vac-verify-ci/receipt.json](./proof/vac-verify-ci/receipt.json) is required. Missing or mismatched run_id / job conclusions / harness-gate-attest digests fail-closed. The receipt may bind an ancestor SHA whose green run still matches the GitHub API; it cannot skip the live HEAD attestation. Vercel `timesyncher` / timesyncher.com is a named deploy; doctor and dry-run have no remote target — do not invent one. A Vercel commit status is not vacation-verify.
