...
- Committed inspectable dry-run receipts live under [features/proof/](./proof/) for `telegram-text-single-edit`, `thing-media-stale`, and `thing-media-visible`. Doctor requires those paths. Do not treat a gitignored `artifacts/` run as in-tree proof.
