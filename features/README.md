...
| Jev quality and post-intake | Every generated reply stores a Jev score and a turn-specific comment. A score of 3 or below, or any split or splitting wording, blocks the send until a bake-off rewrite ships. The JSONL stores `shippedModel`, and `draftModel` plus `rewriteModel` when rewritten. A view-or-edit question offers both. The long intake builds the itinerary, sets the trip badge and day placement from customer words, explains collaborators, and gives the one unlimited welcome. A later price question names that price without a second welcome. Onboarding, Itinerary, and each Thing page with collaborator notes are in the app | `jev-quality-post-intake.md` |
