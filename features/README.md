...
- SHA evidence for the reviewer commands is a GitHub Actions `vacation-verify` run bound to this SHA: doctor requires parsed `run:` steps (not whole-file string includes) plus a recorded `run_id` with `conclusion=success` (or a verified in-progress live run / committed `features/proof/vac-verify-ci/receipt.json` for this SHA). Fail-closed if missing. The job uploads `doctor.json`, fixture `events.jsonl`, and `proof_digest` receipts. Vercel `timesyncher` / timesyncher.com is a named deploy; doctor and dry-run have no remote target — do not invent one. A Vercel commit status is not vacation-verify.
