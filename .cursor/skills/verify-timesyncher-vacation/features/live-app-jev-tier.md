...
- `tier-reply` stores the tiered-model text as the app turn from `vacation-app-reply-rules`. At most one full collaborator and access welcome is stored, and only after the customer asks about price, access, or joining. That welcome keeps the exact phrase `unlimited vacations for the whole year`. Day advice does not append it. A Big Island trip stays on the Big Island for the whole session.
