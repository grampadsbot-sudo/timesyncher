...
Require `ok: true`, Feature Map present, this skill present, pipeline syntax valid, the fixture catalog complete, workflow `run:` steps that actually invoke the five reviewer commands, and a SHA-bound `vacation-verify` `run_id` (`conclusion=success`, or the live in-progress job for this SHA). Hosted Vercel `timesyncher` is a deploy status, not a remote doctor/dry-run target. If doctor fails, stop. Do not dry-run a broken catalog.
...
- reviewer commands must run as GitHub Actions vacation-verify on the SHA; doctor binds run_id/conclusion, not yaml text presence; Vercel deploy status is not that evidence
