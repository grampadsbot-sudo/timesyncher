...
const trekAgentEdit = await readFile(new URL('../scripts/trek-agent-edit.mjs', import.meta.url), 'utf8');
const vacationEditPipeline = await readFile(new URL('../scripts/vacation-edit-pipeline.mjs', import.meta.url), 'utf8');
const productDispatch = await readFile(new URL('../scripts/product-gbrain-dispatch.mjs', import.meta.url), 'utf8');
...
assert.match(sharedAudioBridge, /vacation-edit-pipeline\.mjs/);
assert.match(sharedAudioBridge, /pageContext/);
assert.match(vacationEditPipeline, /boundedParserPrompt/);
assert.match(vacationEditPipeline, /Current page\/trip items JSON/);
assert.match(vacationEditPipeline, /Omeke\/Omeker for Umekes/);
assert.match(vacationEditPipeline, /trek-itinerary-edit\.mjs/);
assert.match(vacationEditPipeline, /trek-agent-edit\.mjs/);
assert.match(vacationEditPipeline, /I heard "\$\{heard\}", couldn't find a match, what do you mean\?/);
assert.match(vacationEditPipeline, /checking or researching it is not a supported itinerary change yet/);
assert.match(vacationEditPipeline, /coordinatedDeleteRequests/);
assert.match(vacationEditPipeline, /deterministic_coordinated_delete_parser/);
assert.match(vacationEditPipeline, /runnerPlanOverrideForBoundedIntent/);
assert.match(vacationEditPipeline, /TIMESYNCHER_TREK_AGENT_EDIT_FAKE_RESULT/);
assert.match(vacationEditPipeline, /take\\s\+off/);
assert.match(vacationEditPipeline, /'--max-turns', '3'/);
assert.match(vacationEditPipeline, /Changed \"\$\{before\}\" to \"\$\{after\}\"/);
assert.match(vacationEditPipeline, /Removed \"\$\{title\}\" from the timeline/);
assert.match(productDispatch, /applySharedVacationEditPipeline/);
assert.match(productDispatch, /vacation-edit-pipeline\.mjs/);
assert.match(productDispatch, /pageContext/);
assert.match(trekAgentEdit, /take\\s\+\(\?:out\|off\)/);
assert.match(trekAgentEdit, /planFromBoundedIntent/);
assert.match(trekAgentEdit, /bounded_intent_verified_target/);
assert.match(trekAgentEdit, /isIncompleteMoveRequest/);
assert.match(trekAgentEdit, /incomplete_move_target/);
assert.match(trekAgentEdit, /umekes fish market bar & grill/);
assert.match(trekAgentEdit, /omeker/);
assert.match(trekAgentEdit, /sayHappyHourMatch/);
assert.match(trekAgentEdit, /if \(!ops\.length\n    && \/\\bhappy hour\\b/);
assert.match(trekAgentEdit, /if \(addMatch\?\.\[1\] && !ops\.length\)/);
