...
const validation = validateReceiptForActivation({ session: accepted, receipt: storedReceipt, requiredEulaVersion: '2026-04-initial-draft' });
if (!validation.ok) throw new Error(`activation validation failed: ${validation.errors.join(', ')}`);
const status = activationStatus(store, 'telegram:6373624711', '2026-04-initial-draft');
...
if (stale.ok) throw new Error('stale EULA version should not activate');
