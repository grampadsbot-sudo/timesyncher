...
function renderAcceptanceCopyHtml({ receipt }) {
  return `<!doctype html><html><head><meta charset="utf-8"><title>TimeSyncher Terms Acceptance ${escapeHtml(receipt.eula.version)}</title><style>body{font-family:system-ui,sans-serif;line-height:1.5;margin:40px;max-width:900px}.box{border:1px solid #ccc;padding:16px;border-radius:8px;background:#f8fafc}pre{white-space:pre-wrap}</style></head><body><h1>TimeSyncher Terms Acceptance Copy</h1><div class="box"><p><strong>Client:</strong> ${escapeHtml(receipt.clientLabel)} (${escapeHtml(receipt.clientKey)})</p><p><strong>Accepted by:</strong> ${escapeHtml(receipt.eula.acceptedByName)}</p><p><strong>Accepted at:</strong> ${escapeHtml(receipt.eula.acceptedAt)}</p><p><strong>Terms version:</strong> ${escapeHtml(receipt.eula.version)}</p><p><strong>Terms SHA-256:</strong> ${escapeHtml(receipt.eula.eulaTextSha256)}</p><p><strong>Receipt SHA-256:</strong> ${escapeHtml(receipt.receiptSha256)}</p></div><h2>Included assistance snapshot</h2><pre>${escapeHtml(JSON.stringify(receipt.capabilitySnapshot, null, 2))}</pre><h2>Accepted terms text</h2><pre>${escapeHtml(eulaText)}</pre></body></html>`;
}
