...
export function telegramTurnAfterGate(gate = {}) {
  ...
  if (!gate?.skip && gate?.failClosed) {
    return {
      ...
      reply: gate.receipt?.customer_facing_response || '',
    };
  }
  if (plannedWritesReplied) {
    const heard = noApplyHeard({
      intents: gate.receipt?.intents,
      text: gate.receipt?.transcript,
    });
    return {
