...
const PRINT_MEDIA_CSS_PATCH = '.print-media-card{margin:0 10px 12px 0;width:auto;max-width:48%;display:inline-block;vertical-align:top;text-align:center;break-inside:avoid}.print-media-card>img{width:260px;max-width:100%;height:180px;object-fit:cover;border-radius:10px;border:1px solid #e5e7eb;background:#f8fafc}';
...
export function assertPatchedStyleTwo(source = '') {
  ...
  if (!js.includes('data-cat-keep="1"') || !js.includes('${tsMapsOn?`<div class="map-box" data-category-map="1"')) {
    throw new Error('Category maps must follow the list so a 280px map does not orphan the Restaurants header.');
  }
  ...
  if (!js.includes('<base href=') || !js.includes('width:260px')) {
    throw new Error('Print HTML must resolve /ts-thing-media from origin and embed full-size bound photos.');
  }
  return true;
}
