...
const WD_META_PATCH = 'return`<article class="thing style2-thing" data-thing-card="1"><div class="thing-head"><div class="style2-thing-meta">${an(Mo)}</div>';
...
const STYLE2_DETAILS_PATCH = '.style2-details{display:block!important;column-count:2!important;column-gap:12px;column-fill:auto}.style2-details>.thing,.style2-details>.style2-thing{display:inline-block!important;width:100%!important;max-width:100%!important}';
...
export function assertPatchedStyleTwo(source = '') {
  ...
  if (!js.includes('data-last-content-page="1"') || !js.includes('data-last-page-logo="1"') || !js.includes('justify-content:flex-end')) {
  ...
  if (!js.includes('[data-print-ready=style2] .style2-details') || !js.includes('column-count:2')) {
    throw new Error('Style two Mc() details must pack 2-col continuous cards.');
  }
  if (!js.includes(THING_BREAK_PATCH) || !js.includes(STYLE2_DETAILS_PATCH) || js.includes('.style2-details{display:grid;grid-template-columns:1fr;gap:10px}') || !js.includes('column-count:2') || !js.includes('column-fill:auto')) {
    throw new Error('Style two Mc() must 2-col column-count flow (not TREK 1-col grid + half-blank pages).');
  }
